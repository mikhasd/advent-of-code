...
def load_input(day: int) -> str:
    day_input_path = aoc_2023.joinpath(f'day-{day:02d}.input.txt')
    if not day_input_path.exists():
        raise Exception(f'missing input file day-{day:02d}.input.txt on directory {aoc_2023}')
...
def load_input_lines(day: int) -> Generator[int, None, None]:
    ...
    with day_input_path.open('r') as input_file:
        ...
        while line:
            yield line[:-1] # Remove trailing \n - might break some challenges already developed
            line = input_file.readline()
