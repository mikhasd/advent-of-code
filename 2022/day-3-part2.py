...
def elf_groups() -> List[str]:
    elfs = puzzle_input.splitlines()
    for i in range(int(len(elfs)/3)):
        j = i * 3
        yield [elfs[j], elfs[j+1], elfs[j+2]]
...
def solve():
    ...
    for group in elf_groups():
        [a, b, c] = list(map(set, group))
        badge_item = a.intersection(b).intersection(c).pop()
        priority = priorities[badge_item]
        result += priority
